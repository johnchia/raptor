...
| RCD  | `rcd`  | Raptor Config Daemon. Owns `raptor.conf`: validates every edit against a published schema, applies what the running daemon can take live, writes what it cannot, and sequences the restarts that follow -- including the quiesce bracket that keeps an rvd restart from killing every other MI holder. `rmq` and `raptorctl` are clients; neither writes the file. Serves its key table over `schema`, so a client renders forms and warnings from the camera rather than a second copy of the table. |
| RIC  | `ric`  | IR-Cut Controller. Hybrid luma+gain day/night detection: ae_luma for day-to-night, auto-calibrating gain-ratio for night-to-day (prevents IR flip-flop). Alternative triggers where the board has the hardware: a photoresistor on an ADC (`trigger=adc`) or a digital photosensor on a pin (`trigger=gpio`). Pins from raptor.conf or auto-discovered from `/etc/thingino.json`; single and dual-GPIO filters. |
