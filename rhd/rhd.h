...
/*
 * True for a client that connected, took a slot, and has still not asked
 * for anything. There are only RHD_MAX_CLIENTS slots and nothing else
 * bounds this state: a stream, a parked snapshot, an async job and a
 * draining response each carry their own deadline, so none of them are
 * still reading a request and none are reaped here.
 */
static inline bool rhd_client_recv_expired(const rhd_client_t *c, int64_t now)
{
	if (c->is_mjpeg || c->is_audio || c->send_buf || c->snap_pending || c->api_job)
		return false;
