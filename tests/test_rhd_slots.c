 ...
 * own deadline? Only the first kind may be dropped for going quiet. The five
...
/*
 * A POST /api/v1/rcd asked its question and is waiting on a round trip to
 * another daemon. The client did its part; the delay is ours.
 */
TEST an_api_job_in_flight_is_kept(void)
{
	rhd_client_t c = at_rest();
	int job;
...
SUITE(rhd_slots_suite)
{
	...
	RUN_TEST(an_api_job_in_flight_is_kept);
}
